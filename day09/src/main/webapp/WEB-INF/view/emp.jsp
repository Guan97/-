...
<body class="gray-bg">
        ...
        <div class="row">
			  ...
			  <div class="col-lg-2">
				<div class="input-group">
			      ...
			      <input type="date" placeholder="开始时间" id="txt_search_startbirthday" name="startbirthday" />
                     <input type="date" placeholder="结束时间"id="txt_search_endbirthday" name="endbirthday" />
				</div>
			  </div>
					...
					<form id="form_user" method="post" action="reserveEmp.htm" enctype="multipart/form-data">
						...
						<table style="border-collapse:separate; border-spacing:0px 10px;">
							...
							<tr>
								<td>性别：</td>
								<td colspan="4">
									<select class="form-control" name="sex" id = "sex" aria-required="true" required>
	...
	<!-- 新增部门对话框 -->
	<div class="modal fade" id="modal_user_adds" role="dialog" aria-labelledby="modal_user_adds" aria-hidden="true">
		<div class="modal-dialog">
			<div class="modal-content">
				<div class="modal-body">
					<form id="form_adds" method="post" action="reserveDept.htm">
						...
						<div class="modal-footer">
							<button type="button" class="btn btn-primary"  id="submit_form_adds_btn">保存</button>
							<button type="button" class="btn btn-default" data-dismiss="modal">关闭</button>
						</div>
					</form>
	...
	$(function () {
	    ...
	    //添加部门
	    var validator = $("#form_adds").validate({
    		submitHandler: function(form){
   		      $(form).ajaxSubmit({
   		    	dataType:"json",
   		    	success: function (data) {
	    ...
	    $("#submit_form_adds_btn").click(function(){
	    	$("#form_adds").submit();
	    });
			...
			$.ajax({
			    ...
			    success:function(res){
			    	if(res.success){
			    		var xary=new Array();
			    		var yary=new Array();
			    		res.data.forEach(function(empVo){//  饼图 数据处理
			    			xary.push(empVo.dname);
			    			yary.push( {value: empVo.num, name: empVo.dname});
	...
	var TableInit = function () {
	        ...
	        $('#table_user').bootstrapTable({
	           ...
	           // height: 500,                        //行高，如果没有设置height属性，表格自动根据记录条数觉得表格高度
	            ...
	            },
	            {
	                ...
	                sortable:true,
	            	formatter:function(value,row,index){
	                	return '<img alt="xxx" width="120px" height="100px" src="'+value+'">';
	               		 }
	            }, {
	                field: 'dname',
	                title: '部门',
	    ...
	    oTableInit.queryParams = function (params) {
	        var temp = {//这里的键的名字和控制器的变量名必须一致，这边改动，控制器也需要改成一样的
	            ...
	            startbirthday: $("#txt_search_startbirthday").val(),
	            endbirthday: $("#txt_search_endbirthday").val(),
	...
	var ButtonInit = function () {
	    ...
	    oInit.Init = function () {
	    	...
	    	$("#btn_adds").click(function(){
	    		...
	    		$('#modal_user_adds').modal({backdrop: 'static', keyboard: false});
				$('#modal_user_adds').modal('show');
	        });
